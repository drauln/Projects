...
namespace Szazalek
{
    class Futtathato
    {
        static void Main(string[] args)
        {
            ...
            double min = 1.5;
            double max = 2.2;
            int[] szuletesiIdopontok = new int[elemszam];
            int db = 0;
            double szazalek;
            ...
            for (int i = 0; i < elemszam; i++)
            {
                szuletesiIdopontok[i] = ember[i].SzuletesIdeje.Month;
            }
            
            for (int i = 0; i < elemszam; i++)
            {
                if (szuletesiIdopontok[i]==2 || szuletesiIdopontok[i] == 3)
                {
                    db++;
                }
            }
            szazalek = db  * 100 / elemszam;
            for (int i = 0; i < elemszam; i++)
            {
                Console.WriteLine(ember[i].ToString());
            }
            
            Console.WriteLine("Az emberek " + szazalek + " száézaléka született 2. vagy 3. hónapban.");
